...
***
```{r, echo = F, results = "hide", warning=F, message=F, fig.height=7, fig.width=10, out.width="1920px",out.height="1080px"}
ggthemr_reset()
...
graphFeature <- function(feature = "Nuclear Energy Consumption PPPD (kWh)")
{
  bp %>% filter_(paste(ticker(feature), ">", 0)) %>%
    ...
    theme(legend.position = "none") + labs(title = paste("Importance of ", feature %>% str_extract("\\w+"), " for deep decarbonization."))
}
...
graphFeature(featss[1]) + gg
```

***
```{r, fig.height=6, fig.width=10, echo = F, results = "hide", warning=F, message=F}
ggthemr_reset()
